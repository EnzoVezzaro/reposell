...
### Changed
- **GitHub Device Flow**: replaced PAT-based auth with zero-server GitHub Device Flow on the landing page — no Cloudflare Worker or client secret needed
- Sell page template now sourced from `@reposell/sell` package instead of hardcoded strings
- "Your contribution" renamed to "Buyer's contribution" in docs

### Added
- `@reposell/sell` npm package: canonical sell page template (CSS, JS, storefront schema, render function)
- GitHub OAuth callback page at `/auth/github/callback`
